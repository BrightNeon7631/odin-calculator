...
let ifEquals = 0; // i.e. function that displays the result after pressing '=' hasn't run
let firstNumber = null;
let secondNumber = null;
let operatorWhichButton = 0; // stores which operator button has been selected
let isOperatorButtonClickedFirstTime = 0;
let backspaceBlockIfButtonSelected = 0; // if this value is equal to 1 it stops the 'backspace' button from reducing the value on the 'screen'
...
function showValue(e) {
    ...
    backspaceBlockIfButtonSelected = 0;
...
function whichButton(e) {
    ...
    if (isOperatorButtonClickedFirstTime === 0) {
        ...
        operatorWhichButton = e.target.value; 
        backspaceBlockIfButtonSelected = 1;
        ...
        eqOperator.textContent = operatorButtonWhichToSymbol(operatorWhichButton);
        secondNumber = display.textContent;
        eqSecond.textContent = secondNumber;
        // prevents dividing by 0; displays a message and resets the calcualator
        if (operatorWhichButton == "divide" && secondNumber == 0) {
        ...
        firstNumber = operate(operatorWhichButton, firstNumber, secondNumber);
...
function equals() {
        ...
        if (operatorWhichButton === 0 || firstNumber === null || secondNumber === null) {
            return;
        }
...
function dotFunction() {
    if (ifDotExists === 0 && ifEquals === 0 && backspaceBlockIfButtonSelected === 0) {
        displayValue += '.';
        display.textContent = displayValue;
        ifDotExists = 1;
    } else {
...
function back() {
    if (backspaceBlockIfButtonSelected === 0 && ifEquals === 0) {
...
const yellowButtonArray = Array.from(yellowButtons);
